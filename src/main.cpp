...
int main(void)
{
    mfrc522 rfid(new SPI(10, 3000000));//3000000hz
    uint8_t byte;
    uint8_t str[MAX_LEN];
    ...
    byte = rfid.mfrc522_read(VersionReg);// 버전을 읽어준다
    ...
    while (1)
    {
        ...
        printf("byte-->%d ", byte);
        if (byte == CARD_FOUND) //3값이 들어오면 아무값이 없어서 카드값을 읽어온다
        {
        ...
        delay(1000); //1초간격으로 폴링으로 체크하고 있다.
        printf("hihi\n");
    }
}
